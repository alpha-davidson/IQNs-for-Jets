...
def main():
    model_name = "Trig_Gaussian"  # Network name
    ...
    for x in range(0, len(out[:, 0, 0]), 20):
        noise_t = np.random.normal(0, 0.1, size=sample_num)*model_input[x]

        plot_min = func(model_input[x]) - abs(model_input[x])*0.4
        plot_max = func(model_input[x]) + abs(model_input[x])*0.4
