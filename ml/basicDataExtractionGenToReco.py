...
medianPredictionH5 = f.create_earray(f.root, 'medianPrediction', f32Atom, (0, 4))
meanPredictionH5 = f.create_earray(f.root, 'meanPrediction', f32Atom, (0, 4))
genQuanilteH5 = f.create_earray(f.root, 'recoQuantile', f32Atom, (0, 4))
...
physicsFlavorDataH5 = f.create_earray(f.root, 'physicsFlavor', i32Atom, (0, 1))
...
for x in range(totalSamples//batchSize+1):
    ...
    data = data.reshape((data.shape[0]//actualBatchSize, actualBatchSize, 4), order="f")
    allQuants = []
    allMeans = []
    allMedians = []
    for y in range(4):
